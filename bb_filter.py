...
def apply_filters_to_3d_bb(bb_3d_data, depth_array, sensor_width, sensor_height,df_3):
    # Bounding Box processing
    bb_3d_vehicles = bb_3d_data[0]
    bb_3d_walkers = bb_3d_data[1]
    df_3_vehicles = df_3[0]
    df_3_walkers = df_3[1]

    valid_bb_vehicles,valid_df_vehicles = filter_bounding_boxes(bb_3d_vehicles, depth_array, sensor_width,sensor_height, df_3_vehicles, actor='vehicle', )
    valid_bb_walkers, valid_df_walkers = filter_bounding_boxes(bb_3d_walkers, depth_array, sensor_width,sensor_height, df_3_walkers, actor='walker')

    timestamp = round(time.time() * 1000.0)
    valid_bbs = [valid_bb_vehicles, valid_bb_walkers]
    list1=list(valid_bb_vehicles)
    list2=list(valid_bb_walkers)
    list3=list1+list2
    valid_bbs_temp = list3
    valid_df_temp = valid_df_vehicles.append(valid_df_walkers)
    valid_df_temp["twodbboxes"] = valid_bbs_temp
    valid_df = [valid_df_vehicles, valid_df_walkers]

    return valid_bbs, valid_df_temp


def filter_bounding_boxes(bb_data, depth_data, frame_width, frame_height, df_3, actor):
    good_bounding_boxes = []
    idx_list=[]
    idx_list_1=[]
    ...
    for idx, actor_bb_3d in enumerate(bb_data):

        # Apply some medium constraining on the data to not exclude every impossible point
        possible_bb_3d_points = np.array([x for x in actor_bb_3d if bounds[0] <= x[0] <= bounds[1] and bounds[2] <= x[1] <= bounds[3]])
